...
// the following code segment generates a Bloom filter off a predetermined set of words.
...
public class Training {
	...
	public static void main(String[] args) throws Exception {
		...
		// A new BloomFilter object is constructed using the optimal vector size
		// and optimal
		// number of hash functions (k) based on the input parameters.
		...
		for (FileStatus status : fs.listStatus(inputFile)) {
			...
			// Each file returned from listStatus is read line-by-line,
			// and each line is used to train the Bloom filter.
		...
		// a BloomFilter is also a Writable object, serializing it is fairly
		// trivial:

		// use the FileSystem object to create a new FSDataOutputStream -
		// a utility that wraps a OutputStream in a DataOutputStream.
		FSDataOutputStream strm = fs.create(bfFile);

		// pass the stream to the filter’s write method.
		// After all the input files are ready, the Bloom filter is serialized
		// to the filename provided at the command line.
		filter.write(strm);
